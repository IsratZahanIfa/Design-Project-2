...
$stmt2 = $conn->prepare("SELECT name, email, contact FROM sellers WHERE user_id=?");
...
if (isset($_POST['update_info'])) {
    ...
    if ($newFirst == "" || $newLast == "" || $newEmail == "" || $newContact == "") {
        ...
        if ($seller) {
            $stmt2 = $conn->prepare("UPDATE sellers SET name=?, email=?, contact=? WHERE user_id=?");
            $stmt2->bind_param("sssi", $updatedName, $newEmail, $newContact, $user_id);
            $sellerUpdated = $stmt2->execute();
        } else {
            $stmt2 = $conn->prepare("INSERT INTO sellers (user_id, name, email, contact, created_at) VALUES (?, ?, ?, ?, NOW())");
            $stmt2->bind_param("isss", $user_id, $updatedName, $newEmail, $newContact);
            $sellerUpdated = $stmt2->execute();
        }


        if ($userUpdated && $sellerUpdated) {
...
<html>
<head>
<title>Seller Profile Settings</title>
<style>
...
<div class="sidebar">
    ...
    <a href="seller_dashboard.php" class="save-btn">Back to Dashboard</a>
