.avatar{width:70px;height:70px;border-radius:50%;background:#4b4bff;font-size:24px;display:flex;align-items:center;justify-content:center;margin:auto;}
...
$stmt = $conn->prepare("SELECT id, name, email, password, contact, created_at, role FROupdate_info'])) {
...
.profile-card ustomer_dashboard.php" class="save-btn">Back to Dashboard</a>


        <div classM users WHERE id=?");
$stmt->bind_param("i", $user_id);
$stmt->execute();
